...
var PYTORCH_DOCS_ROOT_URL = 'https://pytorch.org/docs/stable/torch.html';
...
async function scrape( options ) {
	...
	candidates = $( '#tensors' ).add( $( '#random-sampling' ) ).add( $( '#math-operations' ) ).add( $( '#utilities' ) ).find( 'dl.function' ).add( 'dl.attribute' );
	...
	for ( i = 0; i < candidates.length; i++ ) {
		...
		name = name;
		desc = replace( sig.next().children().first().text(), /\r?\n/g, ' ' );

		link = sig.find( 'dt a.anchorjs-link' ).attr( 'href' );
		if ( !link ) {
			debug( 'Interface does not have a documentation link. Interface: %s.', name );
		}
